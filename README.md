...
Two things are added: **8-voice polyphony** across the audio tracks, and a **live oscilloscope screen**
with tuner and track activity.
...
## Features (release v3p, unit shows version `1.5b`)
...
**Scope** - opens with the "..." (three dots) key
- Waveform -> X-Y (stereo goniometer) -> close, with the same key. **YES** = fullscreen wave, **NO** = close.
...
- The top bar (pattern, name, tempo), mutes, pattern/bank change, page keys and **knobs** all keep working
  while the scope is open, so you can tweak a page and watch the waveform change.
...
python3 tools/build.py --official <official OS 1.53 .syx> --tool <path to the container tool>
...
| `tests/` | emulator tests: the real firmware code runs under unicorn with the patches (`tests/run_tests.sh`) |
...
| release | unit shows | hardware |
|---|---|---|
| v3p | 1.5b | not yet tested (knobs through the scope) |
| v3o | 1.5a | not yet tested (tuner) |
| v3n | 1.5Z | **confirmed** - recommended fallback |
