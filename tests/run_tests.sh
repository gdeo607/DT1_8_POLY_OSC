...
# Usage: tests/run_tests.sh <official section_3_MAIN_OS.bin> <patched section_3 .bin> [scope|spectrum]
# Needs: python3, pip install unicorn pillow, and m68k-linux-gnu-nm (binutils) for symbol lookups.
set -e
OFF="$1"; NEW="$2"; PAGE="${3:-scope}"
[ -f "$OFF" ] && [ -f "$NEW" ] || { echo "usage: $0 <official section_3.bin> <patched section_3.bin> [scope|spectrum]"; exit 2; }
cd "$(dirname "$0")"; BIN=../bin
if [ "$PAGE" = spectrum ]; then
